...
PROTO_PATH := https://github.com/protocolbuffers/protobuf/releases/download/v29.0/
ifeq ($(UNAME_S), Linux)
	PROTO_ZIP= protoc-29.0-linux-x86_64.zip
endif
ifeq ($(UNAME_S), Darwin)
	PROTO_ZIP= protoc-29.0-linux-x86_64.zip
endif
...
PROTO_DEFS_CSPROTO := ./httpgrpc/httpgrpc.proto
...
.tools/bin/protoc-gen-go: .tools
	GOPATH=$(CURDIR)/.tools go install google.golang.org/protobuf/cmd/protoc-gen-go@latest

.tools/bin/protoc-gen-go-grpc: .tools
	GOPATH=$(CURDIR)/.tools go install google.golang.org/grpc/cmd/protoc-gen-go-grpc@latest

.tools/bin/protoc-gen-fastmarshal: .tools
	GOPATH=$(CURDIR)/.tools go install github.com/CrowdStrike/csproto/cmd/protoc-gen-fastmarshal@latest
