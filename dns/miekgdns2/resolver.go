...
func (r *Resolver) LookupSRV(_ context.Context, service, proto, name string) (cname string, addrs []*net.SRV, err error) {
	...
	return r.lookupSRV(servers, conf, service, proto, name)
}

func (r *Resolver) lookupSRV(servers []string, conf *dns.ClientConfig, service, proto, name string) (cname string, addrs []*net.SRV, err error) {
	var target string
	if service == "" && proto == "" {
		target = name
	} else {
	...
	response, err := r.query(servers, conf, target, dns.Type(dns.TypeSRV))
	if err != nil {
		return "", nil, err
	}
...
func (r *Resolver) LookupIPAddr(_ context.Context, host string) ([]net.IPAddr, error) {
	...
	// Close connections to nameservers no longer configured in resolv.conf
	r.client.Clean(servers)
	return r.lookupIPAddr(servers, conf, host, 1, 8)
}

func (r *Resolver) lookupIPAddr(servers []string, conf *dns.ClientConfig, host string, currIteration, maxIterations int) ([]net.IPAddr, error) {
	...
	response, err := r.query(servers, conf, host, dns.Type(dns.TypeAAAA))
	if err != nil || len(response.Answer) == 0 {
		// Ugly fallback to A lookup.
		response, err = r.query(servers, conf, host, dns.Type(dns.TypeA))
		if err != nil {
			return nil, err
		}
	}
	...
	for _, record := range response.Answer {
		...
		case *dns.CNAME:
			...
			addrs, err := r.lookupIPAddr(servers, conf, addr.Target, currIteration+1, maxIterations)
			if err != nil {
...
func (r *Resolver) query(servers []string, conf *dns.ClientConfig, name string, qType dns.Type) (*dns.Msg, error) {
	...
	for i := 0; i < conf.Attempts; i++ {
		for _, server := range servers {
			...
			if response.Truncated {
				merr.Add(fmt.Errorf("resolution against server %s for %s: %w", server, name, ErrResponseTruncated))
				continue
			}
...
func (p *Pool) Get(ctx context.Context, network string, server string) (*dns.Conn, error) {
	...
	if p.closed {
		return nil, fmt.Errorf("%w: pool for %s %s is closed", ErrPoolClosed, network, server)
	}
