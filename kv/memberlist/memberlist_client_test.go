...
func TestBasicGetAndCas(t *testing.T) {
	...
	mkv := NewKV(cfg, log.NewNopLogger(), &providerImpl{})
...
func TestJoinMembersWithRetryBackoff(t *testing.T) {
	...
	for i, port := range ports {
		...
		mkv := NewKV(cfg, log.NewNopLogger(), &providerImpl{}) // Not started yet.
...
func TestMultipleCodecs(t *testing.T) {
	...
	mkv1 := NewKV(cfg, log.NewNopLogger(), &providerImpl{})
	...
	mkv2 := NewKV(cfg, log.NewNopLogger(), &providerImpl{})
...
func TestRejoin(t *testing.T) {
	...
	mkv1 := NewKV(cfg1, log.NewNopLogger(), &providerImpl{})
	require.NoError(t, services.StartAndAwaitRunning(context.Background(), mkv1))
	defer services.StopAndAwaitTerminated(context.Background(), mkv1) //nolint:errcheck

	mkv2 := NewKV(cfg2, log.NewNopLogger(), &providerImpl{})
	require.NoError(t, services.StartAndAwaitRunning(context.Background(), mkv2))
	defer services.StopAndAwaitTerminated(context.Background(), mkv2) //nolint:errcheck
...
func TestNotifyMsgResendsOnlyChanges(t *testing.T) {
	...
	kv := NewKV(cfg, log.NewNopLogger(), &providerImpl{})
...
func (p *providerImpl) Resolve(ctx context.Context, addrs []string) error {
	p.resolved = addrs
	return nil
}

func (p providerImpl) Addresses() []string {
	return p.resolved
}
