...
func (t *PropagationTracker) running(ctx context.Context) error {
	level.Info(t.logger).Log("msg", "propagation tracker started", "beacon_interval", t.beaconInterval)
...
// maybePublishBeacon checks if a recent beacon exists. If not, publishes a new one.
func (t *PropagationTracker) maybePublishBeacon(ctx context.Context) {
	...
	// Check if any active beacon was published recently
	for _, beacon := range desc.Beacons {
		if beacon.DeletedAt != 0 {
			continue
		}
		age := now.Sub(beacon.GetPublishedAtTime())
		if age < t.beaconInterval {
			// Recent beacon exists, don't publish
			return
		}
	}

	// No recent beacon, publish a new one
	t.publishBeacon(ctx)
}
