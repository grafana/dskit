...
func TestPropagationTracker_TracksPropagationDelay(t *testing.T) {
	...
	tracker1 := NewPropagationTracker(mkv, beaconInterval, log.NewNopLogger(), registry1)
	...
	tracker2 := NewPropagationTracker(mkv, beaconInterval, log.NewNopLogger(), registry2)
...
func TestPropagationTracker_SkipsTrackingPropagationDelayOnInitialSync(t *testing.T) {
	...
	publisher := NewPropagationTracker(mkv, beaconInterval, log.NewNopLogger(), prometheus.NewPedanticRegistry())
	...
	tracker := NewPropagationTracker(mkv, beaconInterval, log.NewNopLogger(), registry)
...
func TestPropagationTracker_PublishesFirstBeacon(t *testing.T) {
	...
	tracker := NewPropagationTracker(mkv, 100*time.Millisecond, log.NewNopLogger(), prometheus.NewPedanticRegistry())
...
func TestPropagationTracker_OneBeaconPerIntervalPerCluster(t *testing.T) {
	...
	for i := 0; i < numTrackers; i++ {
		tracker := NewPropagationTracker(mkv, beaconInterval, log.NewNopLogger(), prometheus.NewPedanticRegistry())
		require.NoError(t, services.StartAndAwaitRunning(context.Background(), tracker))
