...
/*
BasicLifecycler is a BasicService that is responsible for publishing changes to a ring for a single instance.
...
Rather, it's the delegate's responsibility to call [BasicLifecycler.ChangeState].
  ...
  - The lifecycler will broadcast heartbeats and state changes based on the [ring.BasicLifecyclerConfig.HeartbeatPeriod], calling [ring.BasicLifecyclerDelegate.OnRingInstanceHeartbeat] each time.
