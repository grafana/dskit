...
func TestMerge(t *testing.T) {
	...
	{ // idempotency: (no change after applying same ring again)
		...
		assert.EqualExportedValues(t, expectedFirstSecondMerge(), our)
	...
	{ // associativity: Merge(Merge(first, second), third) == Merge(first, Merge(second, third))
		...
		assert.EqualExportedValues(t, expectedFirstSecondThirdMerge(), our2)
	}

	{
		out, ch := merge(expectedFirstSecondThirdMerge(), fourthRing())
		assert.EqualExportedValues(t, expectedFirstSecondThirdFourthMerge(), out)
...
func TestTokensTakeover(t *testing.T) {
	...
	{ // idempotency: (no change after applying same ring again)
		...
		assert.EqualExportedValues(t, merged(), our)
		assert.Equal(t, (*Desc)(nil), ch)
	}
...
func TestMergeLeft(t *testing.T) {
	...
	{ // associativity: Merge(Merge(first, second), third) == Merge(first, Merge(second, third))
		our1, _ := merge(firstRing(), secondRing())
		our1, _ = merge(our1, thirdRing())
		assert.EqualExportedValues(t, expectedFirstSecondThirdMerge(), our1)
...
func TestMergeRemoveMissing(t *testing.T) {
	...
	{ // commutativity is broken when deleting missing entries. But let's make sure we get reasonable results at least.
		...
		assert.EqualExportedValues(t, &Desc{
			Ingesters: map[string]*InstanceDesc{
				"Ing 1": {Addr: "addr1", Timestamp: now, State: InstanceState_ACTIVE, Tokens: []uint32{30, 40, 50}},
				"Ing 2": {Addr: "addr2", Timestamp: now + 5, State: InstanceState_ACTIVE, Tokens: []uint32{5, 10, 20, 100, 200}},
