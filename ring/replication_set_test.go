...
func TestDoForReplicationSets(t *testing.T) {
	// Ensure none of these tests leak goroutines.
	t.Cleanup(func() {
		goleak.VerifyNone(t)
	})

	generateCallbackFunction := func() func(context.Context, ReplicationSet) ([]string, error) {
		return func(_ context.Context, set ReplicationSet) ([]string, error) {
			return set.GetIDs(), nil
		}
	}

	t.Run("should return no results and no error on no input replication sets", func(t *testing.T) {
		actual, err := DoForReplicationSets[string](context.Background(), nil, generateCallbackFunction())
		require.NoError(t, err)
		assert.Empty(t, actual)
	})

	t.Run("should just run the function on the input replication set if the list has 1 entry", func(t *testing.T) {
		sets := []ReplicationSet{
			{Instances: []InstanceDesc{{Id: "1"}, {Id: "2"}}},
		}

		actual, err := DoForReplicationSets[string](context.Background(), sets, generateCallbackFunction())
		require.NoError(t, err)
		assert.Equal(t, []string{"1", "2"}, actual)
	})

	t.Run("should call the function once for each input replication set and merge the result", func(t *testing.T) {
		sets := []ReplicationSet{
			{Instances: []InstanceDesc{{Id: "1"}, {Id: "2"}}},
			{Instances: []InstanceDesc{{Id: "3"}}},
			{Instances: []InstanceDesc{{Id: "4"}, {Id: "5"}}},
		}
	...
	t.Run("should call the function concurrently for every replication set", func(t *testing.T) {
		sets := []ReplicationSet{
			{Instances: []InstanceDesc{{Id: "1"}}},
			{Instances: []InstanceDesc{{Id: "2"}}},
			{Instances: []InstanceDesc{{Id: "3"}}},
			{Instances: []InstanceDesc{{Id: "4"}}},
			{Instances: []InstanceDesc{{Id: "5"}}},
		}

		startTime := time.Now()
		actual, err := DoForReplicationSets[string](context.Background(), sets, func(_ context.Context, set ReplicationSet) ([]string, error) {
	...
	t.Run("should return as soon as the first error is returned by the callback function", func(t *testing.T) {
		...
		startTime := time.Now()
		actual, err := DoForReplicationSets[string](context.Background(), sets, func(ctx context.Context, set ReplicationSet) ([]string, error) {
			defer wg.Done()

			if len(set.GetIDs()) == 1 && set.GetIDs()[0] == "1" {
	...
	t.Run("should not leak goroutines when all callback functions return an error nearly at the same time", func(t *testing.T) {
		...
		_, err := DoForReplicationSets[string](context.Background(), sets, func(ctx context.Context, set ReplicationSet) ([]string, error) {
