...
// GenerateTokens returns tokensCount unique tokens, none of which clash with the given takenTokens.
// The optimal number of tokens (optimalTokenPerInstance), i.e., 512, reserved for the underlying
// instance are generated by generateAllTokens. GenerateTokens selects the first tokensCount tokens
// from the reserved tokens set, that are not already present in the takenTokens.
// Returned tokens are sorted.
// If it is not possible to return the requested amount of tokens, GenerateTokens panics. This happens
...
func (t *SpreadMinimizingTokenGenerator) GenerateTokens(tokensCount int, takenTokens []uint32) Tokens {
	if tokensCount > optimalTokensPerInstance {
		err := fmt.Errorf("the requested number of tokens (%d) is too high: maximal allowed number of tokens is %d", tokensCount, optimalTokensPerInstance)
		panic(err)
	}
	...
	if len(tokens) == tokensCount {
		return tokens
	}
	err := fmt.Errorf("the requested number (%d) of unique tokens not included in the given set of tokens was too high: only %d tokens is available", tokensCount, len(tokens))
	panic(err)
}
