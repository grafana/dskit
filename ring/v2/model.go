	...
	if x != nil {
		if x.Ingesters != nil {
			if ptr, ok := x.Ingesters[id]; ok {
				return *proto.Clone(ptr).(*InstanceDesc), true
			}
		}
	}
...
func (x *Desc) GetIngesterVals() map[string]InstanceDesc {
	if x != nil {
		cloneMap := make(map[string]InstanceDesc, len(x.Ingesters))
		for k, v := range x.Ingesters {
			cloneMap[k] = *proto.Clone(v).(*InstanceDesc)
		}
		return cloneMap
	}
	return nil
...
func (d *Desc) SetIngesterVal(id string, ingester InstanceDesc) InstanceDesc {
	...
	clone := proto.Clone(&ingester).(*InstanceDesc)
	d.Ingesters[id] = clone
	return *clone
}
...
func (d *Desc) FindIngestersByState(state InstanceState) []InstanceDesc {
	...
	for _, ing := range d.Ingesters {
		if ing.State == state {
			ingClone := proto.Clone(ing).(*InstanceDesc)
			result = append(result, *ingClone)
		}
	}
	return result
}
...
func (d *Desc) TokensFor(id string) (myTokens, allTokens Tokens) {
	...
	ingester := d.GetIngester(id)
	myTokens = ingester.Tokens
	return
}
