...
func normalizedSource() *Desc {
	r := NewDesc()
	r.SetIngesterVal("first", InstanceDesc{
		Tokens: []uint32{100, 200, 300},
	})
	r.SetIngesterVal("second", InstanceDesc{})
	return r
}
...
func TestClaimTokensFromNormalizedToNormalized(t *testing.T) {
	...
	assert.EqualExportedValues(t, normalizedOutput(), r)
}
...
func TestDesc_Ready(t *testing.T) {
	...
	r.SetIngesterVal("some ingester", InstanceDesc{
		Tokens:    []uint32{12345},
		Timestamp: now.Unix(),
	})
