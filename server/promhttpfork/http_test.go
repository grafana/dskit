...
func TestHandlerErrorHandling(t *testing.T) {
	...
	// same registry in the promhttp.HandlerOpts.
	...
	errorHandler := HandlerForTransactional(mReg, promhttp.HandlerOpts{
		ErrorLog:      logger,
		ErrorHandling: promhttp.HTTPErrorOnError,
		Registry:      reg,
	})
	continueHandler := HandlerForTransactional(mReg, promhttp.HandlerOpts{
		ErrorLog:      logger,
		ErrorHandling: promhttp.ContinueOnError,
		Registry:      reg,
	})
	panicHandler := HandlerForTransactional(mReg, promhttp.HandlerOpts{
		ErrorLog:      logger,
...
func TestInstrumentMetricHandler(t *testing.T) {
	reg := prometheus.NewRegistry()
	mReg := &mockTransactionGatherer{g: reg}
	handler := InstrumentMetricHandler(reg, HandlerForTransactional(mReg, promhttp.HandlerOpts{}))
	// Do it again to test idempotency.
	InstrumentMetricHandler(reg, HandlerForTransactional(mReg, promhttp.HandlerOpts{}))
	writer := httptest.NewRecorder()
	request, _ := http.NewRequest(http.MethodGet, "/", nil)
...
func TestHandlerTimeout(t *testing.T) {
	reg := prometheus.NewRegistry()
	handler := HandlerFor(reg, promhttp.HandlerOpts{Timeout: time.Millisecond})
...
func TestHandlerWithMetricFilter(t *testing.T) {
	...
	mReg := &mockTransactionGatherer{g: reg}
	handler := HandlerForTransactional(mReg, promhttp.HandlerOpts{})
