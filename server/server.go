...
func (cfg *Config) RegisterFlags(f *flag.FlagSet) {
	...
	f.DurationVar(&cfg.HTTPServerReadTimeout, "server.http-read-timeout", 30*time.Second, "Read timeout for HTTP request. This timeout covers entire HTTP request, unless -server.http-read-header-timeout is set, in which case this timeout is only applied after reading request headers.")
	f.DurationVar(&cfg.HTTPServerReadHeaderTimeout, "server.http-read-header-timeout", 0, "Read timeout for HTTP request headers. If set to 0, reading of headers is included in -server.http-read-timeout.")
