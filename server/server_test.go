...
// TestHTTPWriteTimeout tests the default behavior of HTTPServerWriteTimeout which is generally not desirable
// - it hard cuts off the TCP connection which causes the client to error
// - it does not cancel the context of the request handler and instead waits for the next write to close the tcp conn
// Compare this to TestHTTPWriteTimeoutWithTimeoutHandler below
func TestHTTPWriteTimeout(t *testing.T) {
	var writeTimeout = time.Second
	var httpDelayTimeout = 5 * time.Second

	var cfg Config
	cfg.RegisterFlags(flag.NewFlagSet("", flag.ExitOnError))
	cfg.HTTPListenPort = 9090
	cfg.HTTPServerWriteTimeout = writeTimeout
	cfg.MetricsNamespace = "testing_http_write_timeout"
	...
	server.HTTP.HandleFunc("/test", func(w http.ResponseWriter, r *http.Request) {
		select {
		case <-time.After(httpDelayTimeout):
		case <-r.Context().Done():
			require.FailNow(t, "context was cancelled but should not have been")
		}

		w.WriteHeader(204)

		// the context is not in error even though the write timed out b/c of the way the http write timeout works
	...
	// this will timeout BUT it will take 5 seconds to return b/c of the way server write timeout behaves
	start := time.Now()
	req, err := http.NewRequest("GET", "http://127.0.0.1:9090/test", nil)
	require.NoError(t, err)
	...
	require.Greater(t, duration, 3*time.Second) // this took over 3 seconds to return even though the write timeout is  1 second
}

// TestHTTPWriteTimeoutWithTimeoutHandler tests the behavior of using an http.TimeoutHandler which is generally more desirable
// - it returns a meaningful http response instead of just dropping the tcp connection
// - it cancels the context of the request handler so downstream work can be cancelled
func TestHTTPWriteTimeoutWithTimeoutHandler(t *testing.T) {
	var writeTimeout = time.Second
	var httpDelayTimeout = 5 * time.Second
	...
	cfg.HTTPListenPort = 9090
	cfg.HTTPServerWriteTimeout = writeTimeout
	cfg.HTTPServerUseTimeoutHandler = true
	cfg.HTTPServerTimeoutHandlerMessage = "server timed out!"
	cfg.MetricsNamespace = "testing_http_write_timeout_with_handler"
	...
	server.HTTP.HandleFunc("/test", func(w http.ResponseWriter, r *http.Request) {
		select {
		case <-time.After(httpDelayTimeout):
			require.FailNow(t, "timeout was hit, but context should have been cancelled first")
		case <-r.Context().Done():
		}

		w.WriteHeader(204)

		// the context is in error b/c the timeouthandler cancelled it
	...
	// this will timeout after one second b/c the handler cancelled it
	start := time.Now()
	req, err := http.NewRequest("GET", "http://127.0.0.1:9090/test", nil)
	require.NoError(t, err)
	resp, err := http.DefaultClient.Do(req)
	require.NoError(t, err) // client returns no error, but now returns a meaningful http response

	body, err := io.ReadAll(resp.Body)
	require.NoError(t, err)
	require.Equal(t, http.StatusServiceUnavailable, resp.StatusCode)
	require.Equal(t, "server timed out!", string(body))
	defer resp.Body.Close()

	duration := time.Since(start)
	require.Less(t, duration, 3*time.Second) // this took less than 3 seconds to return b/c it correctly bailed out on the request
}
