// Provenance-includes-location: https://github.com/cortexproject/cortex/blob/v1.10.0/pkg/util/active_user.go
...
// NewActiveUsers returns an empty activity tracker.
...
// UpdateUserTimestamp records activity using the same timestamp unit as PurgeInactiveUsers.
...
// NewActiveUsersCleanupWithDefaultValues checks every three minutes for users inactive for fifteen minutes.
...
// NewActiveUsersCleanupService calls cleanupFn for expired users at each cleanup interval.
// The callback runs synchronously and must not be nil.
...
// UpdateUserTimestamp records the latest activity time for a user.
...
// ActiveUsers returns an unordered snapshot of users that have not been purged.
func (s *ActiveUsersCleanupService) ActiveUsers() []string {
	...
	for user := range s.activeUsers.timestamps {
