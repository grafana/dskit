...
func SubtenantID(ctx context.Context) (string, string, error) {
	...
	for {
		...
		// The orgID is guaranteed to already validated.
		if part != orgID {
			return "", "", user.ErrTooManyOrgIDs
		}
		if sepIdx == -1 {
...
// a normalized list of ordered and distinct subtenant IDs (as produced by
// NormalizeTenantIDs). Empty subtenants are omitted from the result.
//
// ignore stutter warning
//
//nolint:revive
...
type Resolver interface {
	...
	// tenant ID. It returns an error user.ErrNoOrgID if there is no tenant ID
	// supplied or user.ErrTooManyOrgIDs if there are multiple tenant IDs present.
	// If a subtenant is present in the orgID, it is stripped and only the tenant ID is returned.
	TenantID(context.Context) (string, error)

	// TenantIDs returns all tenant IDs from the context. It should return
	// normalized list of ordered and distinct tenant IDs (as produced by
	// NormalizeTenantIDs).
	// If a subtenant is present in the orgID, it is stripped and only the tenant IDs are returned.
